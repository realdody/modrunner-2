import { Collection, SlashCommandBuilder, EmbedBuilder, inlineCode, ChannelType, ChatInputCommandInteraction, TextChannel, Guild } from 'discord.js';
...
const command: Command = {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        const projectList = new Collection<string, { projectId: string; channels: string[]; projectName: string }>();
        ...
        const firstPage = new EmbedBuilder()
            ...
            .setDescription(
                `**Projects: ${projects.length}${channel ? '' : '/'}${channel ? '' : guildSettings?.maxProjects
        ...
        logger.debug(
            `${interaction.user.tag} requested the tracked projects list for their guild ${interaction.guild.name
        ...
        for (let i = 0; i < projectList.size; i++) {
            ...
            projectList.at(i)!.channels.forEach((ch) => {
                const channel = interaction.client.channels.cache.get(ch) as TextChannel;
