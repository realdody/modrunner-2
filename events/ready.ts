...
// @ts-ignore
import { getMods } from '../api/curseforge';
// @ts-ignore
...
import { sendUpdateEmbed } from '../utils';
...
async function checkForProjectUpdates(client: Client) {
    ...
    const dbCurseforgeProjectsIds: number[] = [];
    for (const dbProject of dbCurseforgeProjects) {
        ...
        dbCurseforgeProjectsIds.push(parseInt(dbProject.id));
    }
    ...
    if (dbCurseforgeProjects.length) {
        for (const dbProject of dbCurseforgeProjects) {
            ...
            const requestedMod = requestedMods.data.find((element: any) => element.id.toString() === dbProject.id);
            if (!requestedMod) {
...
async function updatePresenceData(client: Client) {
...
async function sweepDatabase(client: Client) {
    ...
    let realModrinthProjects: any[] = [];
    if (data && data.statusCode === 200) realModrinthProjects = await getJSONResponse(data.body);
